...
def log_reg(input_X: Input[Dataset], 
    input_y: Input[Dataset],
    output: Output[Model],
    output_metrics: Output[ClassificationMetrics]):
    import os
...
def forest(input_X: Input[Dataset], 
    input_y: Input[Dataset],
    output: Output[Model],
...
def crowning(
    ...
    output_challenger: Output[Model]) -> NamedTuple(
    "Outputs",
    [
    ...
    return (output_champion.path.replace("/gcs/", "gs://"), output_challenger.path.replace("/gcs/", "gs://"))
...
def pipeline(
    ...
    model_1 = KNN(train_test.outputs['output_X_train'], train_test.outputs['output_y_train'])
    model_2 = log_reg(train_test.outputs['output_X_train'], train_test.outputs['output_y_train'])
    model_3 = forest(train_test.outputs['output_X_train'], train_test.outputs['output_y_train'])
    model_4 = svm(train_test.outputs['output_X_train'], train_test.outputs['output_y_train'])
    final_models = crowning(model_1.outputs['output'], model_2.outputs['output'], model_3.outputs['output'], model_4.outputs['output'], train_test.outputs['output_X_test'], train_test.outputs['output_y_test'])
    absolute_result = absolute_testing(final_models.outputs['output_champion'], final_models.outputs['output_challenger'], df_scaled.outputs['output_scaled'])
    for i in final_models.outputs:
        print(i)
    with dsl.Condition(
        absolute_result.output == 'champion'):
    ...
    with dsl.Condition(
        absolute_result.output == 'challenger'):
        model_upload_op_1 = import_model(
            project_id=project,
            region=region,
